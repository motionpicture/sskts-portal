...
function getSmartHeader(){
	...
	if($arr["ename"] == "top" && !preg_match("!news/detail!",$Url)){
		$html = "";
		$html .= "<!-- header -->";
		$html .= "<h1 id='header'><img src='$define[Images_SP_URL]common/header_rogo.gif' width='119' height='24' alt='シネマサンシャイン'></h1>";
		$html .= "<!-- header -->";
	}else{
		$html = "";
		$html .= "<!-- header -->";
		$html .= "<div id='header'><a href='#' onClick='history.back(); return false;'><img class='btn_return' src='$define[Images_SP_URL]common/header_return.gif' width='27' height='24' alt='戻る'></a>";
...
function getSmartFooter(){
	...
	if($arr["ename2"]){
		...
		$menu .= "<li><a class='bottom_line' href='$define[GROBAL_SP_TOP_URL]theater/$arr[ename]/access/'>アクセス</a></li>";
		$menu .= "<li><a class='bottom_line' href='$define[GROBAL_TOP_URL]question/'>よくあるご質問</a></li>";
		$menu .= "</ul>";
	}
