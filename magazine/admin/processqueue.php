...
//echo "select id,userselection,rsstemplate,subject from ".$tables["message"]." where status != \"draft\" and status != \"sent\" and status != \"prepared\" and status != \"suspended\" and embargo < now() order by entered";
...
while ($message = Sql_fetch_array($messages)) {
  ...
  //var_dump("select count(*) from ".$tables["attribute"]);
	//echo strlen($userselection);
	//var_dump($userselection);
