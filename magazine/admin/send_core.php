...
#    print $_POST["attribute_names[".$_POST["criteria_attribute"]."]"];
  ...
  if ($send && $subject && $from && $message && !$duplicate_atribute && sizeof($_POST["targetlist"])) {
    if ($status == "submitted") {
      print "<h3>メッセージの配信処理準備が終了しました。</h3>";
      print '<p>'.PageLink2("processqueue","メッセージの送信処理").'</p>';
    }
    $done = 1;
    return;
...
#    }
    ...
    $tabs->addTab("２．属性選択","$baseurl&amp;tab=Criteria");
    //$tabs->addTab($GLOBALS['I18N']->get("Lists"),"$baseurl&amp;tab=Lists");
    $tabs->addTab("３．メールマガジン選択","$baseurl&amp;tab=Lists");
#    $tabs->addTab("Review and Send","$baseurl&amp;tab=Review");
  ...
  function savechanges() { if (changed) { if (confirm("
		  <?php echo str_replace('"','&quot',reverse_htmlentities($GLOBALS['I18N']->get("unsavedchanges")))?>
...
//  $formatting_content .= '>';
  ...
  #0013076: different content when forwarding 'to a friend'
  $tmp = '<tr><td colspan=2>'.' '.$GLOBALS['I18N']->get("message").'. </td></tr>
...
#    $maincontent .= $oFCKeditor->ReturnFCKeditor( 'message', $w.'px', $h.'px' ) ;
  ...
  $sendtest_content = sprintf('<hr /><table><tr><td valign="top">
    ...
    <td><input type=text name="testtarget" size=40 value="'.$_POST["testtarget"].'">
    </td></tr></table><hr />',
...
#  $values_drop .= '<span id="values_select">';
  ...
  '<span class="criteria_element" style="visibility:hidden">'.$operator_drop.'</span>'.
