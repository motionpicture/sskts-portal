...
class PHPlistMailer extends PHPMailer {
    ...
    #  parent::SetLanguage('en', dirname(__FILE__) . 'phpmailer/language/');
      ...
      $this->addCustomHeader("X-Mailer: CinemaSunshine");
    ...
    function add_text($text) {
      if (!$this->Body) {
        ...
        $this->Body = html_entity_decode($text ,ENT_QUOTES, 'iso-2022-jp' ); #$text;
#        $this->Body = $text;
       } else {
        //$this->AltBody = html_entity_decode($text ,ENT_QUOTES, 'UTF-8' );#$text;
        $this->AltBody = html_entity_decode($text ,ENT_QUOTES, 'iso-2022-jp' );#$text;
      }
    }
