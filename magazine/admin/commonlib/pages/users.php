...
print '<div id="not_confirmed">現在'. "$totalunconfirmed"."人 のユーザが仮登録状態です。"."</div>";
...
<select name="findby">
<option value="email" <?php echo $findby == "email"? 'selected="selected"':''?>><?php echo $GLOBALS['I18N']->get('Email')?></option>
...
while ($row = Sql_Fetch_Array($att_req)) {
	printf('<option value="%d" %s>%s</option>', $row["id"], $row["id"] == $findby ? 'selected="selected"' : '', $row["name"]);
}
...
if ($result)
	while ($user = Sql_fetch_array($result)) {
		...
		if (in_array("blacklist", $columns)) {
			...
			$ls->addColumn($user["email"], "メールマガジン解除", $onblacklist ? $GLOBALS["img_tick"] : $GLOBALS["img_cross"]);
		}
#		$ls->addColumn($user["email"], $GLOBALS['I18N']->get('del'), sprintf('<a href="%s" onclick="return deleteRec(\'%s\');">del</a>',PageUrl2('users'.$find_url), PageURL2("users&start=$start&delete=" .$user["id"])));
		...
		if (in_array("messages", $columns)) {
			...
			$ls->addColumn($user["email"], 
			//$GLOBALS['I18N']->get('msgs')
			"配信数"
...
print $ls->display();
	print('<br />');
	print('<p><a href="./?page=members&id=1">PCメールマガジンリストユーザを見る</a></p>');
	print('<p><a href="./?page=members&id=2">モバイルマガジンリストユーザを見る</a></p>');
	
if (!$some && !$total) {
	//print '<p class="information">' . $GLOBALS['I18N']->get('No users apply') . '</p>';
	print '<p class="information">該当するユーザがいません。</p>';
}
?>
