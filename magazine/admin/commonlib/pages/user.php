...
if ($id) {
 ...
 // printf('&nbsp;&nbsp;<a href="%s">%s</a>',getConfig("unsubscribeurl").'&amp;uid='.$user["uniqid"],$GLOBALS['I18N']->get('unsubscribe page'));
  ...
  while (list ($key,$val) = each ($struct)) {
    ...
    //var_dump($key);
    if ($key == "htmlemail" || $key == "subscribepage" ||$key == "password" ||$key == "passwordchanged" ||
    $key == "foreignkey" ||  $key == "uniqid" ||  $key == "rssfrequency" ||  $key == "extradata"
    ||  $key == "disabled" || $key =="bouncecount") {
    ...
    if ($key == "confirmed") {
      if (!$require_login || ($require_login && isSuperUser())) {
        ...
        //printf('<tr><td>%s (1/0)</td><td><input type="text" name="%s" value="%s" size="5" /></td></tr>'."\n","メールマガジン配信可否",$key,htmlspecialchars($user[$key]));
      } else {
      ...
      printf('<tr><td>%s</td><td>%s</td></tr>',"メールマガジン配信可否",$temp_val);
    } else {
      if (!strpos($key,'_')) {
        if (ereg("sys",$a))
  ...
  while ($row = Sql_fetch_array($res)) {
    ...
    if ($row["type"] == "date") {
      ...
      printf('<tr><td>%s<!--%s--></td><td>%s</td></tr>'."\n",stripslashes($row["name"]),$row["value"],$date->showInput($row["name"],"",$row["value"]));
  ...
  while ($row = Sql_Fetch_Array($req)) {
    ...
    printf ('<td bgcolor="%s"><input type="checkbox" name="subscribe[]" value="%d" %s /> %s</td>',
      $bgcol,$row["id"],$subs,PageLink2("members",stripslashes($row["name"]),"id=".$row["id"]));
  }
  print '</tr>';
  if ($access != "view")
