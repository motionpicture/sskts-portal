...
class WebblerListing {
  ...
  function listingStart() {
    return '<table cellpadding="0" cellspacing="0" border="0" width="100%" class="wordBreak">';
  }
  ...
  function listingElement($element) {
    ...
    if ($element["url"]) {
      $html .= sprintf('<td valign="top" %s class="listingname"><a href="%s" class="listingname">%s</a></td>',$width,$element["url"],$element["name"]);
    } else {
      $html .= sprintf('<td valign="top" %s class="listingname"><div class="listingname">%s</div></td>',$width,$element["name"]);
    }
...
class WebblerTabs {
  ...
  function display() {
	...
	if($Statu != null){
	    ...
	    $html .= '<p style="color:red;">現在Status状態のメールが' . $Statu . '通あります</p>';
	}else{
	    $html .= '<div id="webblertabs">';
	}
...
'.$this->dividerRow().'
  ...
  */
  ...
  function footer() {
    return '</table><!-- End table from header -->';
  }
