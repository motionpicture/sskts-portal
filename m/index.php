...
<?php
	...
	if ($top_imgs['m_url'] != null) {
		echo "<a href='$top_imgs[m_url]'><img src='../image_crop.php?image=$define[GROBAL_TOP_URL]theaters_image/topslider/$top_imgs[pic_path]&wsize=240&hsize=97' alt='" . htmlspecialchars(mb_convert_encoding($top_imgs["name"],"SJIS","UTF-8"), ENT_QUOTES) . "' /></a><br />";
	} else {
		echo "<img src='../image_crop.php?image=$define[GROBAL_TOP_URL]theaters_image/topslider/$top_imgs[pic_path]&wsize=240&hsize=97' alt='" . htmlspecialchars(mb_convert_encoding($top_imgs["name"],"SJIS","UTF-8"), ENT_QUOTES) . "' /><br />";
	}
