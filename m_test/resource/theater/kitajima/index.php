 ...
 * */
$newsList = array();
$lines = @file("{$BASE}/../data/kitajima/news.csv");
if (is_array($lines)) {
...
foreach ($newsList as $value) {
    if (!empty($value[2])) {
        $html .= "<span style=\"color:#000000;\">{$value[1]}</span><br />
<a href=\"./news/detail/?i={$value[0]}\"><span style=\"color:#7c6845;\">{$value[2]}</span></a><br /><br />
";
    }
}
 ...
 * */
$news = false;
$lines = @file("{$BASE}/../data/kitajima/advance.csv");
if (is_array($lines)) {
//	$lines = array_reverse($lines);
    //表示数変更の場合下記
    $count = 10000;
