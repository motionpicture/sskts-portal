<?php
if(preg_match("!/matsudo/!",$_SERVER["REQUEST_URI"])){
	header('Location: http://www.cinemasunshine.co.jp/m/theater/redirect.php?p=1');
    exit();
}elseif(preg_match("!/iwai/!",$_SERVER["REQUEST_URI"])){
	header('Location: http://www.cinemasunshine.co.jp/m/theater/redirect.php?p=2');
    exit();
}
/*
 * Emoji変換用スクリプト
...
$indexFileName = $resourcePath . 'index.xhtml';
...
if(preg_match("!/imabari/!",$resourcePath)){
	$fileName = $resourcePath . 'thanks' . '.xhtml';
}
