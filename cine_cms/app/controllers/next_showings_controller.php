<?php
class NextShowingsController extends AppController {

	var $name = 'NextShowings';
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			...
			if ($this->NextShowing->save($this->data)) {
				$this->Session->setFlash(__('Coming soonが修正されました。', true));
				$this->redirect(array('action' => 'edit/'.$this->data['NextShowing']['theater_id']));
			} else {
		...
		//劇場名とID挿入
		if ($id == null) {
			$id =$this->data['NextShowing']['theater_id'];
		}

		if (empty($this->data)) {
			$this->data =  $this->NextShowing->findByTheaterId($id);

			//劇場追加対応
			if(!$this->data) {
				$this->data['NextShowing']['theater_id']=$id;
			}
		}
		...
		if ($id == "1000") {
			//劇場id
			$this->data['NextShowing']['theater_id']=1000;

			//劇場名挿入
			$this->set('theaterName','シネマサンシャイン トップ');
		} else {
			//劇場id
			$this->data['NextShowing']['theater_id']=$theater['Theater']['id'];
